...
function be_sample_metaboxes_strength( $meta_boxes ) {
	...
	$meta_boxes[] = array(
		...
		'fields' => array(
			...
			array(
				'name' => 'Price',
				'desc' => 'USD 01.00 month',
				'id' => $prefix .$posttype.'price',
				'type' => 'text_money'                               
			),
                    array(
				'name' => 'Diapay Status',
...
if($_POST['post_type'] == 'strength'){
    ...
    if($_POST['original_publish']== 'Publish' )
    {
        ...
        $plan->unit_amount_in_cents->addCurrency('USD', str_replace('.','',$_POST['_cmb_strength_price'])); // USD 10.00 month
    ...
    if($_POST['original_publish']== 'Update' )
    {
        ...
        if(!empty($planCode)){
                  ...
                  //  pr($_POST,1);
                     $planName = $planName->name.' '.$_POST['post_title'];
                     $plan->unit_amount_in_cents['USD']  =  str_replace('.','',$_POST['_cmb_strength_price']); // EUR 50.00 monthly fee
                     $plan->description                  =  $_POST['content'];
                     $plan->name                         =  substr($planName, 0, 250); 
                     $plan->update();
        }
