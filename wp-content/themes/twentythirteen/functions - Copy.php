...
function userEmailRegister()
{
	...
	if(username_exists($getEmail) || email_exists($getEmail))
		{ 	
			$response["message"]="E-mail id already exists!!";
			$response["response"] = 'error';
		}
		else{	
				$rcode = wp_generate_password(6);;
				...
				$subject = 'INSIV CODE';
				$message = 'Hi,<br>';
				$message .= sprintf(__('Code: %s'), $rcode) . "\r\n\r\n";
...
if(!empty($codeResult))
{
		print_r($codeResult->ID);
		
		$newpassword = wp_generate_password(9);
		wp_update_user( array ( 'ID' => $codeResult->ID, 'user_url' => $website ) ) ;
...
//		$login_data['user_password'] = $password;  
	...
	$response = array('response'=>'sucess', 'message'=> $newpassword);	
}else{
